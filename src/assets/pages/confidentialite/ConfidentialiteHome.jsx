const ConfidentialiteHome = () => {
  return (
    <section
      className="confidentialite-home"
      aria-labelledby="confidentialite-title"
    >
      <div className="container">
        <div className="legal-content">
          <h2 id="confidentialite-title">Politique de confidentialité</h2>
          <span>Dernière mise à jour : 26 Juin 2026</span>
          <p>
            Le cabinet SOCRATEA veille à la protection de vos données
            personnelles. <br />
            Cette page vous explique quelles informations peuvent être
            recueillies lorsque vous consultez le site, utilisez le formulaire
            de contact ou prenez rendez-vous, pourquoi elles sont utilisées,
            combien de temps elles sont conservées et quels sont vos droits.
          </p>
          <h2 className="premier-h2">Responsable du traitement</h2>
          <p>
            Le responsable des traitements réalisés dans le cadre du site
            socratea.net est : <br />
            <strong>SOCRATEA</strong> <br />
            Société d'expertise comptable <br />
            21 rue Charlemagne <br />
            80480 Vers-sur-Selle <br />
            Représentée par son gérant : <strong>Julien Jacquesson</strong>{" "}
            <br />
            Adresse électronique :{" "}
            <strong>
              <a href="mailto:julien.jacquesson@socratea.net">
                julien.jacquesson@socratea.net
              </a>
            </strong>{" "}
            <br />
            Téléphone : <strong className="strong-tel">
              07 68 77 55 24
            </strong>{" "}
            <br />
            Pour toute question relative au traitement de vos données
            personnelles ou à l’exercice de vos droits, vous pouvez contacter
            SOCRATEA aux coordonnées indiquées ci-dessus.
          </p>
          <h2>Données susceptibles d’être collectées</h2>
          <p>
            Selon la manière dont vous utilisez le site, SOCRATEA peut
            recueillir les informations suivantes :{" "}
          </p>
          <ul>
            <li>votre nom et votre prénom ;</li>
            <li>le nom de votre entreprise ;</li>
            <li>vos coordonnées professionnelles ;</li>
            <li>
              les informations renseignées dans le formulaire de contact ;
            </li>
            <li>le contenu de votre demande ou de votre message ;</li>
            <li>
              les informations que vous fournissez lors de la prise de
              rendez-vous ;
            </li>
            <li>
              les données techniques nécessaires au fonctionnement et à la
              sécurité du site, telles que l’adresse IP, le navigateur utilisé,
              la date et l’heure de connexion.
            </li>
          </ul>
          <p>
            SOCRATEA vous invite à ne transmettre que les informations
            nécessaires au traitement de votre demande et à ne pas communiquer
            de données sensibles dans le formulaire de contact.
          </p>
          <h2>Finalités et bases juridiques</h2>
          <p>
            Les données collectées peuvent être utilisées pour : <br />
            <strong className="titre-strong">
              Répondre à vos demandes
            </strong>{" "}
            <br />
            Les informations envoyées par le formulaire ou par courrier
            électronique permettent à SOCRATEA de comprendre votre demande et de
            vous répondre. <br />
            Lorsque votre demande concerne une éventuelle mission, le traitement
            repose sur les démarches précontractuelles réalisées à votre
            initiative. <br />
            Pour les autres demandes, le traitement repose sur l’intérêt
            légitime de SOCRATEA à répondre aux personnes qui le contactent.{" "}
            <br />
            <strong className="titre-strong">
              Organiser un rendez-vous
            </strong>{" "}
            <br />
            Les informations renseignées lors de la prise de rendez-vous
            permettent de réserver un créneau, de préparer l’échange et de vous
            adresser les confirmations ou rappels nécessaires. <br />
            Ce traitement repose sur les démarches précontractuelles réalisées à
            votre initiative. <br />
            <strong className="titre-strong">
              Assurer le fonctionnement et la sécurité du site
            </strong>{" "}
            <br />
            Certaines données techniques peuvent être traitées afin de permettre
            l’affichage du site, de détecter d’éventuelles anomalies et de
            prévenir les usages frauduleux. <br />
            Ce traitement repose sur l’intérêt légitime de SOCRATEA à assurer le
            bon fonctionnement et la sécurité de son site. <br />
            <strong className="titre-strong">
              Respecter une obligation légale
            </strong>{" "}
            <br />
            Certaines informations peuvent être conservées ou communiquées
            lorsque la loi, une autorité compétente ou la défense des droits de
            SOCRATEA l’impose. <br />
          </p>
          <h2>Formulaire de contact</h2>
          <p>
            Le formulaire de contact vous permet de transmettre une demande à
            SOCRATEA. <br />
            Les champs signalés comme obligatoires sont nécessaires pour
            comprendre votre demande et pouvoir vous répondre. En leur absence,
            le formulaire ne pourra pas être envoyé ou votre demande ne pourra
            pas être traitée correctement. <br />
            L'envoi du formulaire est assuré au moyen du service{" "}
            <strong className="strong-cliquable">
              <a
                href="https://www.emailjs.com/legal/privacy-policy/"
                target="_blank"
                rel="noopener noreferrer"
              >
                EmailJS
              </a>
            </strong>
            , qui intervient comme prestataire technique afin de transmettre
            votre message à la messagerie de SOCRATEA. <br />
            Les informations communiquées sont utilisées pour prendre
            connaissance de votre demande, vous répondre et assurer le suivi des
            échanges qui peuvent en découler. Elles ne sont pas utilisées pour
            vous adresser une newsletter ou des communications commerciales
            automatiques. <br />
            Pour en savoir plus, vous pouvez consulter la politique de
            confidentialité d’EmailJS.
          </p>
          <h2>Prise de rendez-vous avec Cal.eu</h2>
          <p>
            Le site intègre un module de prise de rendez-vous fourni par{" "}
            <strong>Cal.eu</strong>, service proposé par{" "}
            <strong className="strong-cliquable">
              <a
                href="https://cal.com/fr/privacy"
                target="_blank"
                rel="noopener noreferrer"
              >
                Cal.com
              </a>
            </strong>{" "}
            <br />
            Lorsque vous utilisez ce module, les informations nécessaires à
            l’organisation du rendez-vous peuvent être recueillies directement
            par Cal.eu, notamment votre nom, votre adresse électronique, le
            créneau sélectionné, votre fuseau horaire ainsi que les informations
            que vous choisissez de communiquer. <br />
            Ces données sont utilisées afin de vous présenter les créneaux
            disponibles, d’enregistrer votre rendez-vous et de vous adresser les
            confirmations, rappels ou informations nécessaires à son
            organisation. <br />
            Cal.eu peut également traiter certaines données techniques liées à
            l’utilisation du module, telles que l’adresse IP, le navigateur
            utilisé et les informations de connexion nécessaires à son
            fonctionnement et à sa sécurité. <br />
            SOCRATEA reçoit uniquement les informations nécessaires à
            l’organisation et à la préparation du rendez-vous. <br />
            Pour en savoir plus, vous pouvez consulter la politique de
            confidentialité de Cal.com applicable au service Cal.eu.
          </p>
          <h2>Hébergement</h2>
          <p>
            Le site SOCRATEA est hébergé par : <br />
            <strong>OVH SAS</strong> <br />
            2 rue Kellermann, <br />
            59100 Roubaix, <br />
            France. <br />
            Dans le cadre de l’hébergement, du fonctionnement et de la
            sécurisation du site, certaines données techniques peuvent être
            traitées, notamment l’adresse IP, les informations relatives au
            navigateur et à l’appareil utilisé, ainsi que les journaux de
            connexion nécessaires à la détection des erreurs, à la sécurité et à
            la protection du service. <br />
            OVH intervient en qualité de prestataire technique d’hébergement.
          </p>
          <h2>Destinataires des données</h2>
          <p>Les données sont accessibles uniquement : </p>
          <ul>
            <li>
              à Julien Jacquesson et aux personnes habilitées au sein de
              SOCRATEA ;
            </li>
            <li>
              aux prestataires techniques strictement nécessaires au
              fonctionnement du site, à la transmission du formulaire et à la
              prise de rendez-vous ;
            </li>
            <li>
              aux autorités compétentes lorsqu’une obligation légale l’impose.
            </li>
          </ul>
          <p>Les principaux prestataires concernés sont : </p>
          <ul>
            <li>
              <strong>OVH</strong>, pour l’hébergement du site ;
            </li>
            <li>
              <strong>EmailJS</strong>, pour la transmission du formulaire de
              contact ;
            </li>
            <li>
              <strong>Cal.eu</strong>, lorsque vous choisissez de prendre
              rendez-vous.
            </li>
          </ul>
          <p>SOCRATEA ne vend ni ne loue vos données personnelles.</p>

          <h2>Transferts de données hors de l’Espace économique européen</h2>
          <p>
            Certains prestataires techniques utilisés par SOCRATEA, notamment{" "}
            <strong>EmailJS</strong>, peuvent faire intervenir des sociétés ou
            des sous-traitants situés en dehors de l’Espace économique européen.{" "}
            <br />
            Le service <strong>Cal.eu</strong> est présenté par son éditeur
            comme hébergé au sein de l’Union européenne. <br />
            Lorsqu’un transfert de données hors de l’Espace économique européen
            intervient, il est encadré par les mécanismes prévus par la
            réglementation applicable, tels qu’une décision d’adéquation ou des
            clauses contractuelles types approuvées par la Commission
            européenne. <br />
            Vous pouvez contacter SOCRATEA pour obtenir davantage d’informations
            sur les garanties applicables.
          </p>
          <h2>Durées de conservation</h2>
          <p>
            Les données sont conservées pendant une durée adaptée à la finalité
            pour laquelle elles ont été recueillies. <br />
            <strong className="titre-strong">
              Demandes de contact et prises de rendez-vous
            </strong>{" "}
            <br />
            Lorsque la demande ne donne pas lieu à une relation contractuelle,
            les informations communiquées sont conservées pendant la durée
            nécessaire au traitement de la demande, puis pendant une durée
            maximale{" "}
            <strong>
              de trois ans à compter du dernier contact émanant de la personne
              concernée.
            </strong>{" "}
            <br />
            <strong className="titre-strong">
              Relation avec un client
            </strong>{" "}
            <br />
            Lorsque la demande aboutit à une mission, les informations utiles
            sont intégrées au dossier du client. Elles sont conservées pendant
            la durée de la relation, puis pendant les délais imposés par les
            obligations légales, réglementaires, comptables et professionnelles
            applicables au cabinet. <br />
            <strong className="titre-strong">Données techniques</strong> <br />
            Les journaux et données techniques sont conservés pendant la durée
            strictement nécessaire au fonctionnement, à la sécurité et à la
            protection du site, dans la limite des durées appliquées par
            l’hébergeur et des obligations légales applicables.
            <br />
            <strong className="titre-strong">
              Demandes d’exercice des droits
            </strong>{" "}
            <br />
            Les informations nécessaires au traitement d’une demande relative
            aux données personnelles sont conservées pendant la durée nécessaire
            à son traitement et à la justification de la réponse apportée.
          </p>
          <h2>Cookies et autres traceurs</h2>
          <p>
            Le site SOCRATEA n’utilise actuellement aucun cookie publicitaire ni
            outil de mesure d’audience. <br />
            Le module <strong>Cal.eu</strong> intégré à la page de contact peut
            toutefois utiliser des cookies ou des technologies similaires
            nécessaires à son fonctionnement, à la mémorisation de certains
            choix, à la sécurité du service et à la gestion des rendez-vous.{" "}
            <br />
            Les cookies ou traceurs qui ne sont pas strictement nécessaires au
            fonctionnement du service ne doivent être déposés qu’après votre
            accord. Vous devez pouvoir les refuser aussi facilement que les
            accepter et modifier ultérieurement vos choix. <br />
            Si de nouveaux services nécessitant le dépôt de cookies non
            essentiels sont ajoutés au site, un dispositif de recueil et de
            gestion du consentement sera mis en place avant leur activation.
          </p>
          <h2>Sécurité des données</h2>
          <p>
            SOCRATEA met en œuvre des mesures techniques et organisationnelles
            adaptées afin de limiter les risques de perte, d’accès non autorisé,
            de divulgation ou de modification des données personnelles. <br />
            L’accès aux informations est limité aux personnes et prestataires
            qui en ont besoin pour assurer le fonctionnement du site et traiter
            les demandes reçues. <br />
            Aucun système ne pouvant garantir une sécurité absolue, SOCRATEA
            adapte ses mesures en fonction des risques identifiés et de
            l’évolution des outils utilisés.
          </p>
          <h2>Vos droits</h2>
          <p>
            Selon la nature du traitement et dans les conditions prévues par la
            réglementation, vous pouvez demander :{" "}
          </p>
          <ul>
            <li>l’accès aux données qui vous concernent ;</li>
            <li>la rectification de données inexactes ou incomplètes ;</li>
            <li>l’effacement de vos données ;</li>
            <li>la limitation d’un traitement ;</li>
            <li>l’opposition à certains traitements ;</li>
            <li>la portabilité de certaines données ;</li>
            <li>
              le retrait de votre consentement lorsqu’un traitement repose sur
              celui-ci.
            </li>
          </ul>
          <p>
            Vous pouvez exercer vos droits en écrivant à : <br />
            <strong>
              <a href="mailto:julien.jacquesson@socratea.net">
                julien.jacquesson@socratea.net
              </a>
            </strong>{" "}
            <br />
            Votre demande doit permettre de vous identifier suffisamment et de
            préciser le droit que vous souhaitez exercer. Un justificatif
            d’identité pourra être demandé uniquement lorsque cela est
            nécessaire pour éviter une usurpation d’identité. <br />
            SOCRATEA répondra à votre demande dans les délais prévus par la
            réglementation. <br />
            Si vous estimez que vos droits ne sont pas respectés, vous pouvez
            également adresser une réclamation à la{" "}
            <strong>
              <a
                href="https://cnil.fr/fr/professionnel"
                target="_blank"
                rel="noopener noreferrer"
              >
                Commission nationale de l’informatique et des libertés (CNIL)
              </a>
            </strong>
          </p>
          <h2>Absence de décision automatisée</h2>
          <p>
            Les données collectées par le site ne sont pas utilisées pour
            prendre une décision entièrement automatisée produisant des effets
            juridiques ou significatifs à votre égard. <br />
            Elles ne sont pas utilisées pour établir un profil commercial
            automatisé.
          </p>
          <h2>Liens vers des sites tiers</h2>
          <p>
            Le site peut contenir des liens vers des plateformes ou sites
            exploités par des tiers, notamment Cal.eu ou LinkedIn. <br />
            Lorsque vous accédez à ces services, leurs propres conditions
            d’utilisation et politiques de confidentialité s’appliquent. <br />
            SOCRATEA n’est pas responsable des traitements réalisés directement
            par ces services pour leurs propres finalités.
          </p>
          <h2>Modification de la politique</h2>
          <p>
            La présente politique peut être mise à jour afin de tenir compte :
          </p>
          <ul>
            <li>de l’évolution du site ;</li>
            <li>
              de l’ajout, du remplacement ou du retrait d’un prestataire ;
            </li>
            <li>d’une évolution des traitements réalisés ;</li>
            <li>d’une modification légale ou réglementaire.</li>
          </ul>
          <p>La date de dernière mise à jour figure en haut de la page.</p>

          <h2>Contact</h2>
          <p>
            Pour toute question concernant cette politique ou l’utilisation de
            vos données personnelles : <br />
            <strong>Julien Jacquesson — SOCRATEA</strong> <br />
            21 rue Charlemagne <br />
            80480 Vers-sur-Selle <br />
            Adresse électronique :{" "}
            <strong>
              <a href="mailto:julien.jacquesson@socratea.net">
                julien.jacquesson@socratea.net
              </a>
            </strong>{" "}
            <br />
            Téléphone : <strong className="strong-tel">07 68 77 55 24</strong>
          </p>
        </div>
      </div>
    </section>
  );
};

export default ConfidentialiteHome;
